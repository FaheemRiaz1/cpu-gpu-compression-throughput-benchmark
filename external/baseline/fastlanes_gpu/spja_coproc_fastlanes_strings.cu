...
// FastLanes decodes one 1024-value vector with a single 32-thread warp.
// The SPJA kernel uses a wider block because it operates on decoded rows.
...
// Host-side description of one FastLanes-compressed fact column.
// The encoded payload stays compressed until the CPU or GPU path decodes it.
struct ColumnData {
...
// Averaged timing, work distribution, and correctness data for one CPU/GPU split.
struct SplitResult {
...
// Load one encoded FastLanes column together with its metadata and byte counts.
// This setup is performed before the timed co-processing experiments.
...
// Decode one lane/item value from the FastLanes bit-packed layout on the CPU.
// Values that cross a 32-bit word boundary are reconstructed from two words.
...
// Map a global row to its FastLanes vector, lane, and item and decode that value.
...
// Execute the SPJA query on raw columns to obtain an independent correctness reference.
// Query: quantity > 25 AND market segment == target, then SUM(extendedprice).
...
// Process one CPU-owned row range directly from FastLanes-compressed columns.
// extendedprice is decoded only after the join and predicates succeed.
...
// Divide the CPU-owned vector range across worker threads and combine partial sums.
// The worker count is bounded by the configured CPU limit and the number of rows.
...
// Decode the GPU-owned FastLanes vectors into a contiguous int32 buffer.
// One CUDA block corresponds to one 1024-value FastLanes vector.
...
// Execute the SPJA query over the decoded GPU partition.
// Each CUDA block reduces its qualifying extendedprice values to one partial sum.
...
// Allocate compressed input, decoded columns, and partial-sum storage for one split.
// Allocation remains outside the per-run timed region.
...
// GPU-path timing and aggregate produced by one execution.
struct GpuRunResult {
...
// Execute the complete timed GPU path for one split.
// Timing includes compressed H2D transfer, three FastLanes decodes, SPJA,
// and D2H transfer of the block-level partial sums.
GpuRunResult run_gpu_once(
    ...
    const int* d_customer_mktsegment_code,
  ...
  // Start timing before the compressed GPU-owned slices are transferred.
  ...
  // Decode the three fact columns before launching the relational query.
  ...
  // Query execution follows decompression and produces one sum per CUDA block.
  ...
  // Transfer only the compact block-level aggregates back to the host.
  CUDA_CHECK(cudaMemcpyAsync(
...
// Run one CPU/GPU split with CPU and GPU work executing concurrently.
// End-to-end time is measured directly around the overlap rather than by summing
// the separately observed CPU and GPU path times.
SplitResult run_split(
  ...
  // CPU owns the prefix of FastLanes vectors; GPU owns the remaining suffix.
  ...
  // Reuse the split-specific buffers across warm-up and timed repetitions.
  auto run_once = [&]() -> SplitResult {
    ...
    // The total timer encloses both CPU and GPU paths so overlap is preserved.
  ...
  // Warm-up executions initialize the paths but are excluded from reported averages.
...
// Report logical and encoded sizes for the three FastLanes fact columns.
...
void print_results_table(const std::vector<SplitResult>& results,
  ...
  std::cout << "\nFastLanes SPJA CPU/GPU co-processing results, dictionary-encoded string predicate\n";
  ...
  for (const auto& r : results) {
    ...
    std::cout << std::left << std::setw(12) << "TPCH_X40_STRINGS"
...
// Persist the averaged split results and correctness information.
...
int main() {
  ...
  try {
    ...
    // Encoded fact columns and raw reference columns are prepared before timing.
    ...
    const std::filesystem::path csv_path =
        ...
        "results/fastlanes_lz4nvcomp_dowda/csv/fastlanes_spja_coproc_x40_strings_results.csv";

    // Dictionary code used for the BUILDING market segment.
    const int target_segment = 1;
    ...
    std::cout << "Loaded FastLanes SPJA x40 compressed data with dictionary-encoded string predicate:\n";
    ...
    std::cout << "  String predicate: customer_mktsegment = BUILDING, encoded as segment_code == " << target_segment << "\n";
    ...
    std::vector<int> customer_mktsegment_code =

        read_int_file("data/tpch_columnar/customer_mktsegment_code_sfx40.bin");

    const int order_count = static_cast<int>(order_custkey.size());

    const int customer_count = static_cast<int>(customer_mktsegment_code.size());

    std::cout << "\nComputing raw CPU reference result...\n";

    // Compute the reference from raw data before releasing the large raw fact columns.
    ...
    // Lookup arrays are transferred once and remain resident across all timed splits.
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_customer_mktsegment_code),

                          customer_mktsegment_code.size() * sizeof(int)));
    ...
    CUDA_CHECK(cudaMemcpy(d_customer_mktsegment_code,
    ...
    // Evaluate the same five CPU/GPU split points used by the other baselines.
    ...
    // Running the benchmark rewrites the configured result CSV with current measurements.
...
// Build and run:
//
// From the repository root, use the existing FastLanes build script:
//
// bash scripts/run_fastlanes_spja_x40_strings.sh
//
// The script recompiles this source with the CMake-generated FastLanes include
// configuration, relinks fastlanes_gpu_aggregate, and runs the benchmark.
// The benchmark writes its result CSV to the path defined in main().