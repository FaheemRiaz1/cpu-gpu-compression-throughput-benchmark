...
static constexpr int TARGET_SEGMENT = 1; // BUILDING dictionary code
    ...
    unsigned long long* __restrict__ out_sum)
{
    ...
    for (uint64_t tile = blockIdx.x; tile < n_local_tiles; tile += gridDim.x) {
        ...
        for (int item = 0; item < ITEMS_PER_THREAD; ++item) {
            ...
            if (custkey < 0 || static_cast<uint64_t>(custkey) >= customer_mktsegment_code_n) {
                continue;
            }
            if (customer_mktsegment_code[custkey] != TARGET_SEGMENT) {
                continue;
            }
    ...
    uint64_t end)
{
    ...
    for (uint64_t i = begin; i < end; ++i) {
        ...
        int32_t custkey = order_custkey[ok];
        if (custkey < 0 || static_cast<uint64_t>(custkey) >= customer_mktsegment_code.size()) continue;
        if (customer_mktsegment_code[custkey] != TARGET_SEGMENT) continue;
    ...
    uint64_t end)
{
    ...
    const uint64_t total_rows = end - begin;
    ...
    for (int t = 0; t < actual_threads; ++t) {
        const uint64_t local_begin =
            begin +
            (total_rows * static_cast<uint64_t>(t)) /
            static_cast<uint64_t>(actual_threads);
        const uint64_t local_end =
            begin +
            (total_rows * static_cast<uint64_t>(t + 1)) /
            static_cast<uint64_t>(actual_threads);

        workers.emplace_back([&, t, local_begin, local_end]() {
            partial_sums[static_cast<size_t>(t)] =
                cpu_spja_range_raw(
                    orderkey,
                    quantity,
                    extendedprice,
    ...
    int64_t reference)
{
    ...
    if (cpu_percent > 0) {
        cpu_thread = std::thread([&]() {
            ...
            cpu_sum = cpu_spja_range_parallel(orderkey, quantity, extendedprice,
                                              order_custkey, customer_mktsegment_code,
    ...
    if (gpu_percent > 0 && gpu_start_tile < gpu_end_tile) {
        ...
        dpf_fused_spja_kernel_common<<<grid, BLOCK_THREADS>>>(
            ...
            d_customer_mktsegment_code, customer_mktsegment_code.size(),
            orderkey.size(),
            d_sum);
    ...
    int64_t reference)
{
    ...
    for (int i = 0; i < WARMUP_RUNS; ++i) {
        RunResult warm = run_split_once_common(
            ...
            d_order_custkey, d_customer_mktsegment_code,
    ...
    double compression_reduction)
{
    ...
    for (const auto& r : rows) {
        out << "DPF_FUSED_BINPACK_COMMON_TIMING_STRINGS,"
            << r.cpu_percent << ","
            << r.gpu_percent << ","
...
int main() {
    try {
        ...
        std::cout << "Query: quantity > 25 AND customer_mktsegment = BUILDING, SUM(extendedprice)\n";
        std::cout << "Dictionary encoding: BUILDING -> 1\n";
        ...
        const std::string customer_mktsegment_code_path = base + "customer_mktsegment_code_sfx40.bin";
        ...
        auto customer_mktsegment_code = read_binary_vector<int32_t>(customer_mktsegment_code_path);
        ...
        int32_t* d_customer_mktsegment_code = nullptr;
        unsigned long long* d_sum = nullptr;
        CUDA_CHECK(cudaMalloc(&d_order_custkey, order_custkey.size() * sizeof(int32_t)));
        CUDA_CHECK(cudaMalloc(&d_customer_mktsegment_code, customer_mktsegment_code.size() * sizeof(int32_t)));
        ...
        CUDA_CHECK(cudaMemcpy(d_order_custkey, order_custkey.data(),
                              ...
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_customer_mktsegment_code, customer_mktsegment_code.data(),
                              customer_mktsegment_code.size() * sizeof(int32_t),
                              cudaMemcpyHostToDevice));
        DevicePackedBuffer d_orderkey;
        DevicePackedBuffer d_quantity;
        DevicePackedBuffer d_extendedprice;
        ...
        std::string csv_path =
            "results/fastlanes_lz4nvcomp_dowda/csv/dpf_fused_spja_x40_common_timing_strings_results.csv";
