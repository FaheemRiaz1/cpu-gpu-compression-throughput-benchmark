...
# Repository-relative locations for the benchmark CSVs and generated figures.
# The script compares the same five CPU/GPU splits across all three systems.
...
# Normalize CSV headers so result files with slightly different naming
# conventions can be processed through the same plotting path.
...
# Resolve one logical metric from a list of accepted column names.
# This keeps the comparison script tolerant of schema differences between
# the LZ4/nvCOMP, FastLanes, and DPF result files.
...
# Treat the common correctness markers used by the benchmark CSVs as valid.
def truthy_match(value) -> bool:
    s = str(value).strip().upper()
    return s in {"YES", "Y", "TRUE", "1", "MATCH", "VALID"}


# Convert numeric split percentages to the canonical labels used on the plots.
def split_label(cpu_percent: float, gpu_percent: float) -> str:
    return f"{int(round(cpu_percent))}CPU/{int(round(gpu_percent))}GPU"


# Load one system's result CSV, keep only valid benchmark rows, and derive
# per-device throughput from the fraction of the logical input assigned to it.
def load_csv_for_system(system_name: str, path: Path) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        # Component throughput uses only the logical input fraction owned by
        # that processor rather than the full query input size.
    ...
    # Restore the fixed experimental split order before combining systems.
...
# Draw compact values inside visible stacked-bar segments. Tiny segments are
# intentionally left unlabeled to avoid clutter and unreadable text.
...
# Plot CPU and GPU components side by side for all three compression systems.
# Stacking is used only to visualize the component breakdown; because CPU and
# GPU work can overlap, the stacked height must not be read as E2E runtime.
def plot_stacked(df_all: pd.DataFrame, metric: str):
    ...
    # Select the measured component columns and explanatory text for the
    # requested execution-time or component-throughput view.
    if metric == "execution":
        y_cpu_col = "cpu_ms"
        y_gpu_col = "gpu_ms"
        title = (
        ...
        footnote = (
            "Compression time is excluded. CPU and GPU component times are shown as stacked segments "
            "for visual comparison; stacked height is not end-to-end time because CPU/GPU execute concurrently."
        )
        out_path = OUT_EXEC
    else:
        y_cpu_col = "cpu_thr"
        ...
        footnote = (
            ...
            "Segments are stacked only for visual comparison and are not an additive system-throughput total."
        )
        out_path = OUT_THROUGHPUT

    # Use one shared axis so all systems are directly comparable at each split.
    ...
    # Offset the three system bars around each split while stacking the CPU
    # and GPU components within each individual system bar.
    for system, offset in zip(SYSTEMS, offsets):
        ...
        # CPU is the lower segment and GPU is stacked directly above it.
        b_cpu = ax.bar(
            x + offset,
            cpu_vals,
            width,
    ...
    fig.tight_layout()
    # Save publication-quality figures into the repository results directory.
    fig.savefig(out_path, dpi=220, bbox_inches="tight")
    plt.close(fig)


# Entry point: load all three benchmark result sets and generate the two
# comparison figures from the existing CSV snapshots.
def main():
    ...
    # Combine normalized rows only after each source has passed its own
    # correctness filtering and schema resolution.
    ...
    # Generate both views from the same normalized benchmark data.
...
# Run directly from the repository root so all relative paths resolve correctly.
if __name__ == "__main__":
    main()

# Run from the repository root:
#
# python3 results/fastlanes_lz4nvcomp_dowda/graph_plotting/plot_coproc_stacked_bar.py
#
# The script reads the existing benchmark CSVs and rewrites execution_time.png
# and throughput_time.png under results/fastlanes_lz4nvcomp_dowda/graphs/.
